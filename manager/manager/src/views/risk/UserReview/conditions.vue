<template>
    <div>
        <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item class="subscription-plan">风控条件</el-breadcrumb-item>
        </el-breadcrumb>
        <div class="content">
            ...
            >
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <el-tooltip class="item" effect="dark" :content="scope.row.description" placement="bottom-start">
                            <div 
                                class="desc"
                ...
                >
                    <template slot-scope="scope">
                        <div 
                            v-for="(item, index) in JSON.parse(scope.row.rule).ruleConfig.filter(item => item.value)"
                            :key="index"
                        >
                            {{LOGIC_TYPE.find(type => type.value == item.value).label}}：{{item.description}}
                        </div>
                    </template>
                </el-table-column>
                <el-table-column
...
<script>
import { selectCooperationServiceName, getConditions, updateRiskConditionRule } from '@/api/risk/userReview'
...
export default {
    ...
    methods: {
        ...
        getDataType(row) {
            let res = '-'
            if(row.riskCooperationServiceName && (row.queryMoney || row.residueSize)) {
        ...
        comfirm(e) {
            ...
            updateRiskConditionRule(data).then(res => {
                if (res.code == 0) {
