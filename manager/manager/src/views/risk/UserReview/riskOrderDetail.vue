    ...
    <div>
        ...
        <div class="content">
            ...
            <div class="container" ref="riskContent">
                ...
                <template v-if="businessType == 2 && detail && detail.depositFreeApply && detail.depositFreeApply.addDeposit">
                    <div>
                        <div class="title" style="margin-left:0;">加收押金</div>
                        <div class="item-content" style="display: flex;align-items: center;">
                            需加收保证金 ¥ {{ formatThousandNumber(detail.depositFreeApply.addDeposit) }} 
                            <img :src="passImg" style="margin-left:12px;" v-if="detail.depositFreeApply.isAddDepositAgree === 1">
                            <img :src="refuseImg" style="margin-left:12px;" v-if="detail.depositFreeApply.isAddDepositAgree === 2">
                        </div>
                    </div>
                </template>
                ...
                <template v-if="detail && detail.depositFreeApply && businessType == 1">
                    <div>
                        <!-- <div class="add-materia-btn-wrap" v-if="detail.depositFreeApply.status === 1">
                            <div class="page-btn" @click="openRetrialMaterialDialog">加收材料</div>
                        </div> -->
                ...
                <template v-if="detail && detail.depositFreeApply">
                    <template v-if="detail.depositFreeApply.status === 1 && detail.riskResult === 5">
                        <div>
                            <div class="btn-group" v-if="businessType == 1">
                                <button class="page-btn" :disabled="disabled" @click="openPassDialog">车辆免押申请通过</button>
                                <button class="page-btn" :disabled="disabled" @click="openRefuseDialog">车辆免押申请不通过</button>
                            </div>
                            <div class="btn-group" v-else-if="businessType == 2">
                                <button class="page-btn" :disabled="rentalDisabled" @click="openRentalPassDialog">审核通过</button>
                                <button class="page-btn" @click="openAddMoneyDialog">加收押金</button>
                                <button class="page-btn" :disabled="rentalDisabled" @click="openRentalRefuseDialog">审核不通过</button>
                            </div>
                        </div>
                    </template>
                            ...
                            <div class="items-wrap">
                                <div class="items-line">
                                    <div class="item">
                                        ...
                                        <div class="item-content">{{detail.depositFreeApply.status === 2 ? '通过' : '不通过'}}</div>
                                    </div>
                                    <div class="item">
                                        <div class="item-title">审核人员：</div>
                                        <div class="item-content">{{detail.depositFreeApply.updatedBy || '-'}}</div>
                                    </div>
                                </div>
                                ...
                                <div class="items-line">
                                    <div class="item">
                                        ...
                                        <div class="item-content">{{detail.depositFreeApply.updatedTime || '-'}}</div>
                                    </div>
                                    <div class="item" v-if="detail.depositFreeApply.status === 3">
                                        <div class="item-title">不通过原因：</div>
                                        <div class="item-content">{{detail.depositFreeApply.reason || '-'}}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        >
            ...
            <div class="dialog-wrap">
                <div class="dialog-title">确认通过个人免押申请？</div>
        ...
        >
            ...
            <div class="dialog-wrap">
                <div>
                    <div class="dialog-title">确认拒绝个人免押申请？</div>
            ...
            <div class="guarantor-dialog-wrap">
                ...
                <div style="margin-top:40px">
                    <div class="btn-group" v-if="detail.guarantorInfo.status === 0">
                        <button class="page-btn" :disabled="guarantorDisabled" @click="openGuarantorPassDialog">担保人审核通过</button>
                        <button class="page-btn" :disabled="guarantorDisabled" @click="openGuarantorRefuseDialog">担保人审核拒绝</button>
                    </div>
                    <div v-else>
                        <div class="title">审核信息</div>
                        <div class="items-wrap">
                            <div class="items-line">
                                <div class="item">
                                    <div class="item-title">审核结果：</div>
                                    <div class="item-content">{{detail.guarantorInfo.status === 1 ? '通过' : '不通过'}}</div>
                                </div>
                                <div class="item">
                                    <div class="item-title">审核人员：</div>
                                    <div class="item-content">{{detail.guarantorInfo.updatedBy || '-'}}</div>
                                </div>
                            </div>
                            ...
                            <div class="items-line">
                                <div class="item">
                                    <div class="item-title">审核时间：</div>
                                    <div class="item-content">{{detail.guarantorInfo.updatedTime || '-'}}</div>
                                </div>
                                <div class="item" v-if="detail.guarantorInfo.status === 2">
                                    <div class="item-title">不通过原因：</div>
                                    <div class="item-content">{{detail.guarantorInfo.reason || '-'}}</div>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        >
            ...
            <div class="dialog-wrap">
                ...
                <span slot="footer" class="dialog-footer">
                <div class="dialog-btn-wrap" style="margin-top:40px">
                    <div class="btn" @click="handleCloseGuarantorRefuseDialog">取消</div>
        ...
        <PassDialog :show.sync="passDialog.show" :detail="passDialog" @save="rentalApprove" />
        <RefuseDialog :show.sync="refuseDialog.show" :detail="refuseDialog" @save="rentalApprove" />
        <AddMoneyDialog :show.sync="addMoneyDialog.show" :detail="addMoneyDialog" @save="saveAddDeposit" />
    </div>
</template>

<script>
import { mapState } from 'vuex'
import { selectRiskOrderDetail, certificationConfigList, updateAuditStatusOfAgain, updateAuditStatusOfLog, addUserCarSubscribeCertification, addOtherDeposit, updateGuarantorInfo, updateDepositFreeApply, selectCarRentalRiskOrderDetail, addDeposit, rentalApprove } from '@/api/risk/userReview'
import { RETRIAL_STATUS, CHECKED_STATUS, OCCUPATION_STATUS } from '@/utils/constant'
...
export default {
    ...
    computed: {
        ...
        disabled() {
            ...
            if (this.detail && this.detail.personalQuery && this.detail.personalQuery.length) {
            ...
            if (this.detail && this.detail.companyQuery && this.detail.companyQuery.length) {
            ...
            if (this.detail && this.detail.certificationList && this.detail.certificationList.length) {
                let ruleResultRes = this.detail.certificationList.every(item => item.confirmed == 1)
                if (!ruleResultRes) {
                    res = true
                }
            }

            if (this.detail && this.detail.guarantorInfo && this.detail.guarantorInfo.status !== 1) {
                res = true
            }
        ...
        rentalDisabled() {
            ...
            if (this.detail && this.detail.depositFreeApply && this.detail.depositFreeApply.addDeposit && this.detail.depositFreeApply.isAddDepositAgree !== 1) {
                res = true
            }
        ...
        guarantorDisabled() {
            ...
            if (this.detail && this.detail.guarantorInfo && this.detail.guarantorInfo.personalQuery && this.detail.guarantorInfo.personalQuery.length) {
    ...
    methods: {
        ...
        init() {
            ...
            if (this.detail.companyQuery && this.detail.companyQuery.length) {
                ...
                let stautsRes = this.detail.companyQuery.some(item => item.stauts == 0)
                if (stautsRes) {
                    str = '4'
                }
                ...
                let ruleResultRes = this.detail.companyQuery.some(item => item.ruleResult == 3 || !item.ruleResult)
                if (ruleResultRes) {
                    str = '3'
                }

                ruleResultRes = this.detail.companyQuery.some(item => item.ruleResult == 2)
                if (ruleResultRes) {
                    str = '2'
                }
            ...
            if (this.detail.guarantorInfo && this.detail.guarantorInfo.personalQuery && this.detail.guarantorInfo.personalQuery.length) {
                let str = '1'

                let stautsRes = this.detail.guarantorInfo.personalQuery.some(item => item.stauts == 0)
                if (stautsRes) {
                    str = '4'
                }

                stautsRes = this.detail.guarantorInfo.personalQuery.some(item => item.stauts == 2)
                if (stautsRes) {
                    str = '5'
                }

                let ruleResultRes = this.detail.guarantorInfo.personalQuery.some(item => item.ruleResult == 3 || !item.ruleResult)
                if (ruleResultRes) {
                    str = '3'
                }

                ruleResultRes = this.detail.guarantorInfo.personalQuery.some(item => item.ruleResult == 2)
                if (ruleResultRes) {
                    str = '2'
                }
        ...
        handleScrollToManual() {
            ...
            if (this.customType == '2' && this.detail.compnayRiskRecord.some(item => item.ruleResult == 3)) {
        ...
        updateDepositFreeApply(status) {
            ...
            updateDepositFreeApply(parmas).then((res) => {
                if (res.code == 0) {
                    this.$message.success('保存成功')
                    this.initQuery()
        ...
        updateGuarantorInfo(status) {
            ...
            updateGuarantorInfo(parmas).then((res) => {
                if (res.code == 0) {
                    this.$message.success('保存成功')
                    this.initQuery()
