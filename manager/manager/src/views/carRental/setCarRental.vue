      ...
      <div class="content" v-if="detail">
        ...
        <div class="form-item">
          ...
          <div class="form-content">
            <SelectCarProduct :carTypeYearProductId="detail.carTypeYearProductId" :disabled="type==2" :carEntity="carInfo" @changeCarProduct="changeCarProduct" />
          </div>
        </div>

        <div class="form-item">
          <div class="form-label">新车指导价</div>
          <div class="form-content">{{ carInfo.guidancePrice ? `${formatThousandNumber(carInfo.guidancePrice)}元` : '-' }}</div>
        </div>
        ...
        <div v-for="item in currentCity.typeReqVos" :key="item.period">
          <div class="form-sub-title" v-if="businessType == 1">{{item.period}}天套餐</div>
          <div class="form-item" v-if="businessType == 1">
          ...
          <div class="form-item">
            ...
            <div class="form-content">
              <c-input-number 
                ...
                placeholder="请输入车辆租金"
              >
              </c-input-number> 元/{{businessType == 1 ? '月' : '天'}}
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    ...
    if (this.type == 1) {
      ...
      if (this.businessType == 2) {
        this.defaultCitySubscribe.typeReqVos = [{ value: 1, label: '1天', period: 1, money: undefined, }]
        this.detail.citySubscribeReq = [JSON.parse(JSON.stringify(this.defaultCitySubscribe))]
      }
  ...
  methods: {
        ...
        if (res.code === 0) {
          ...
          this.detail.citySubscribeReq.map(city => {
            if (this.businessType == 1) {
              let typeReqVos = JSON.parse(JSON.stringify(CAR_RENTAL_PERIOD_LIST))
              typeReqVos = typeReqVos.map(type => {
                const findItem = city.typeReqVos.find(period => period.period === type.period)
                if (findItem) {
                  type = findItem
                }
                return type
    ...
    save() {
      ...
      setCarRental(this.detail).then(res => {
        if (res.code === 0) {
