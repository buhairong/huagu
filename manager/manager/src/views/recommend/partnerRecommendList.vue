<template>
  <div>
      <el-breadcrumb separator="/">
          ...
          <el-breadcrumb-item class="subscription-plan">产品推荐</el-breadcrumb-item>
      </el-breadcrumb>
      ...
      <div class="content">
        <div class="create-btn-wrap">
          ...
          <div style="display: flex; align-items:center;">
            <create-button 
              ...
              @create="set(1)"
            />
            <div class="link-text" style="margin-left: 16px;" @click="openLogDialog">操作日志</div>
          </div>
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <div>
                <div class="period" v-for="(item, index) in scope.row.detailList" :key="index">
                  <div v-if="item.monthTotal === 12 && item.paymentType === 0">12期</div>
                  <div v-else-if="item.monthTotal === 6 && item.paymentType === 4">12期</div>
                  <div v-else>6期</div>
                </div>
              </div>
            </template>
          ...
          >
            <template slot-scope="scope">
              <div class="period" v-for="(item, index) in scope.row.detailList" :key="index">
                <div class="payment">{{formatThousandNumber(item.monthPay)}}元/月</div>
                <div class="payment">（{{item.monthTotal === 6 && item.paymentType === 4 ? '分两次付' : '一次性付'}}{{formatThousandNumber(item.totalPayment)}}元/次）</div>
              </div>
            </template>
          </el-table-column>
...
import { selectList, addSubscribeIsRecommend, updateSubscribeIsRecommend, deleteSubscribeIsRecommend } from '@/api/subscribe/subscribe'
  ...
  methods: {
    ...
    del(row) {
      this.$confirm(`确定要删除这个产品吗？`, '提示', {
          ...
          deleteSubscribeIsRecommend(data).then(res => {
            if (res.code === 0) {
