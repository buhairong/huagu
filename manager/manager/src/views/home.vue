    ...
    <el-container>
      <el-header>
        <div class="header">
          ...
          <div class="avatar-wrap" @mouseenter="openUserCenter" @mouseleave="closeUserCenter">
            ...
            >
              ...
              >
                ...
                <el-menu-item
                  ...
                  :key="index"
                  :index="thirdMenu.url"
                  :class="{'active-second-menu': defaultMenu === thirdMenu.url}"
                >
                  {{thirdMenu.name}}
                </el-menu-item>
              </el-submenu>
...
export default {
  data() {
    return {
      ...
      //   },
      // ],
      include: ['openNewCar', 'CarProductTypeManage', 'setCarDeliveryAddress', 'carDetail', 'orderDetail', 'partnerList', 'subscribe'],
  ...
  methods: {
        ...
        if (res.code == 0) {
          ...
          if (!localStorage.getItem("defaultMenu")) {
            ...
            if (res.data[0].thirdMenus && res.data[0].thirdMenus.length) {
              this.defaultMenu = this.defaultMenu ? this.defaultMenu : res.data[0].thirdMenus[0].url
              this.$router.push({
                path: this.defaultMenu
              })
            } else {
              this.defaultMenu = this.defaultMenu ? this.defaultMenu : res.data[0].url
              this.$router.push({
                path: this.defaultMenu
              })
            }
    ...
    changePsd() {
      ...
      updatePassword(this.changePsdForm).then(res => {
        if (res.code === 0) {
...
.quit{
  padding: .2167rem .2833rem .3833rem .2833rem;
