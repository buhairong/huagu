            ...
            >
                <template slot-scope="scope">
                    <div>
                        ...
                        <i style="margin-left:10px"  @click="setSubScribeName(scope.row)" class="el-icon-edit"></i>
                    </div>
                </template>
            </el-table-column>
...
<script>
import { updateCarSubscribeName, deleteCityCarSubscribe, deleteCarSubscribeByCity } from '@/api/subscribe/subscribe'
...
export default {
    ...
    methods: {
        ...
        updateCarSubscribeName(subscribeName, row) {
            ...
            updateCarSubscribeName(params).then(res => {
                if (res.code === 0) {
        ...
        del(row) {
            this.$confirm(`确定要删除这个方案吗？`, '提示', {
                ...
                if (this.type == 1) {
                    deleteCarSubscribeByCity(data).then(res => {
                        if (res.code === 0) {
                    ...
                    deleteCityCarSubscribe(data).then(res => {
                        if (res.code === 0) {
                            this.$message.success('删除成功')
                            this.$emit('search')
                        } else {
