    ...
    methods: {
        ...
        changeCarProduct(e) {
            ...
            if (product) {
                ...
                this.detail.citySubscribeReq[0].purchasingPrice = this.detail.purchasingPrice
                this.detail.citySubscribeReq[0].transactionPrice = this.detail.transactionPrice
            }
        ...
        changeCarColor(e) {
            this.detail.colors = this.colorList.map(item => {
        ...
        delConfig(configId) {
            this.$confirm(`确定要删除这个增配项吗？`, '提示', {
                ...
                const index = this.detail.otherConfigs[0].details.findIndex(item => item.configId === configId)
                if (index !== -1) {
