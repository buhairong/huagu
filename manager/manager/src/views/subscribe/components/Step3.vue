    ...
    <div>
        <el-form ref="step3form" :model="detail" label-width="150px">
            <div class="car-info">
                ...
                {{ detail.carType.carBrand }}·{{ detail.carType.carType }}  {{ detail.carType.carTypeYear }} {{ detail.carType.carTypeYearProduct }}
            </div>
                ...
                <div>
                    <div class="city-tag-list" v-if="detail.citySubscribeReq.length > 1">
                        <div 
                            ...
                            :class="{'active-city-tag': city.cityId == detail.currentCity.cityId}"
                            @click="handleChangeCityTag(city)"
                        >
                            {{ city.cityId ? city.cityName : `其余${detail.otherCitys.length}个城市` }}
            ...
            <el-form-item label="选择上架方案" v-if="detail.newType == 1 && detail.currentCity.typeReqVos && detail.currentCity.typeReqVos.length">
                <div>
                    <div class="plan-list">
                        <div 
                        ...
                        >
                            ...
                            <div class="plan-item">
                                ...
                                <div class="plan-item-price">{{ formatThousandNumber(item.distributionBrokerage) }} 元</div>
                            </div>
            ...
            <el-form-item label="车辆指导价" v-if="detail.newType == 1">
                <div>
                    {{ detail.guidancePrice ? `${formatThousandNumber(detail.guidancePrice)}元` : '输入厂商折扣后自动计算' }}
                </div>
            </el-form-item>

            <el-form-item label="车辆购置税" v-if="detail.newType == 1">
                <div>
                    {{ detail.purchaseTax || detail.purchaseTax === 0 ? `${formatThousandNumber(detail.purchaseTax)}元` : '输入厂商折扣后自动计算' }}
                </div>
            </el-form-item>

            <!-- <el-form-item label="车辆押金">
                <div>
                    {{ detail.currentCity.deposit ? `${formatThousandNumber(detail.currentCity.deposit)}元` : '根据订阅费用自动计算' }}
                </div>
            </el-form-item> -->
            ...
            <el-form-item label="首付比例">
                <div>
                    {{ detail.firstPeriodRate }}%
                </div>
            </el-form-item>

            <el-form-item class="el-form-item_required" label="厂商折扣" v-if="detail.newType == 1">
                <div class="discount-wrap">
                    ...
                    <div class="calc-btn" @click="calc" v-if="detail.calculatorRule === 1">开始测算</div>
                </div>
            </el-form-item>

            <template v-if="detail.calculatorRule === 1">
                <el-form-item class="el-form-item_required" label="车商收购价" v-if="detail.newType == 1">
                    <div>
                        <c-input-number 
                ...
                <el-form-item class="el-form-item_required" label="个人成交价" v-if="detail.newType == 1">
                    <div>
                        <c-input-number 
                ...
                <el-form-item class="el-form-item_required" label="预留利润" v-if="detail.newType == 1">
                    <div>
                        <c-input-number 
            ...
            <el-form-item class="el-form-item_required" label="车辆售价" v-if="detail.newType == 2">
                <div>
                    <c-input-number 
            ...
            <el-form-item class="el-form-item_required" label="12个月后二手车估价" v-if="detail.calculatorRule === 1">
                <div>
                    <c-input-number 
            ...
            <el-form-item label="订阅费用">
                <div>
                    <div v-if="detail.calculatorRule === 2">
                        <div class="apportion-item apportion-item1" v-for="(item, index) in detail.currentCity.apportionFeeRecordList" :key="index">
                            <div style="width: 70px;"> {{`${(index*6)+1}`}}-{{(index*6)+6}}期 </div>
                            <div class="split"></div>
                            <div class="right">
                    ...
                    <div v-else>
                        <div class="apportion-item" v-for="(item, index) in detail.currentCity.apportionFeeRecordList" :key="index">
                            <div style="width: 70px;">{{`${item.period-detail.currentCity.apportionFeeRecordList.length+1}`}}-{{item.period}}期 </div>
                            <div class="split"></div>
                            <div class="right">
...
<script>
import { selectDepositAndMonthPayOfApportionRate, deleteCityCarSubscribe, calculatePurchaseTaxRatio, calculate, selectDepositAndMonthPayOfApportionRatePlanTwo } from '@/api/subscribe/subscribe'
...
export default {
    ...
    methods: {
        ...
        handleChooseCity() {
          ...
          this.detail.currentCity = {
            ...
            this.detail.otherCitys = this.detail.allCitys.filter(item => {
                const index = this.detail.citySubscribeReq.findIndex(city => city.cityId === item.id)
        ...
        handleChangeCityTag(city) {
            this.detail.currentCity = this.detail.citySubscribeReq.filter(item => item.cityId === city.cityId)[0]
        },
        ...
        delCity(city) {
            this.$confirm(`确定要删除 ${city.cityName} 专属方案吗？`, '提示', {
                ...
                const index = this.detail.citySubscribeReq.findIndex(item => item.cityId === city.cityId)
                if (index !== -1) {
        ...
        minus() {
            ...
            this.detail.currentCity.firstPeriodRate = Math.floor((this.detail.currentCity.firstPeriodRate*100 - 1)) / 100
            this.calculate()
        },
        ...
        plus() {
            ...
            this.detail.currentCity.firstPeriodRate = Math.ceil((this.detail.currentCity.firstPeriodRate*100 + 1)) / 100
            this.calculate()
		},
        ...
        calc() {
            if(!this.detail.currentCity.discount && this.detail.currentCity.discount !== 0) {
            ...
            calculate(params).then(res => {
                if (res.code === 0) {
                    ...
                    this.detail.currentCity.residualValueRate = res.data.residualValueRate
                    this.detail.currentCity.twelveMonthMoney = res.data.twelveMonthMoney
                    this.detail.currentCity.deposit = res.data.deposit
                    this.detail.currentCity.distributionBrokerage = res.data.distributionBrokerage
                    this.calculate()
                } else {
        ...
        calculate2() {
            ...
            selectDepositAndMonthPayOfApportionRatePlanTwo(params).then(res => {
                if (res.code === 0) {
                    this.detail.currentCity.apportionFeeRecordList = this.detail.currentCity.apportionFeeRecordList.map((item, index) => {
                        const result = {
                            ...item,
                    ...
                    this.detail.currentCity.typeReqVos = res.data.subscribeMoneyRespVos.map((item, index) => {
                        item.checked = false
                        if (this.detail.currentCity.typeReqVos[index] && this.detail.currentCity.typeReqVos[index].checked) {
                            item.checked = this.detail.currentCity.typeReqVos[index].checked
                        }
                        if (this.detail.currentCity.typeReqVos[index] && this.detail.currentCity.typeReqVos[index].id) {
                            item.id = this.detail.currentCity.typeReqVos[index].id
                        }
                        return item
                    })
                    this.detail.currentCity.deposit = res.data.respVoList[0].deposit
        ...
        calculate1() {
            ...
            selectDepositAndMonthPayOfApportionRate(params).then(res => {
                if(res.code === 0) {
                    ...
                    this.detail.currentCity.distributionBrokerage = res.data.respVoList[0].mouthPayment
                    this.detail.currentCity.deposit = res.data.respVoList[0].deposit
