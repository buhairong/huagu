<template>
    <div>
        <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item class="subscription-plan">{{operator == 1 ? '创建' : '修改'}}方案</el-breadcrumb-item>
        </el-breadcrumb>
        ...
        <div class="content" v-loading="syncLoading" element-loading-text="保存中">
            ...
            <div class="buttom-btn-group">
                ...
                <div class="btn save-btn" @click="save" v-if="activeStep === 3">确认</div>
                <div class="btn save-btn" @click="next" v-else-if="(type == 1 && activeStep === 1) || activeStep === 2">下一步</div>
            </div>
        </div>
    </div>
</template>
...
<script>
import { selectNewCarSubscribeDetail, selectCarSubscribeDetail, calculatePurchaseTaxRatio, getAllCity, insertCarSubscribe } from '@/api/subscribe/subscribe'
...
export default {
    ...
    methods: {
                ...
                if (res.code === 0) {
                    ...
                    res.data = res.data.map(city => {
                        city = {
                            ...city,
                            ...city.detailList[0],
                            firstPeriodRate: city.detailList[0].apportionFeeRecordList[0].rate
                        }
                    ...
                    const service = SERVICE_LIST.map(item => {
                        const findItem = res.data[0].detailList[0].carSubscribeFreeServiceList.find(item1 => item1.serviceName === item.serviceName)
        ...
        next() {
            if (this.activeStep === 0) {
                ...
                if (this.type == 1) {
                    for(const item of this.detail.citySubscribeReq) {
                        ...
                        if (this.detail.calculatorRule === 1) {
                            ...
                            if(!item.twelveMonthMoney && item.twelveMonthMoney != 0) {
                                if (item.cityId) {
                                    this.$message.error(`请设置 ${item.cityName} 的12个月后二手车估价`)
                                } else {
                    ...
                    if(!this.detail.currentCity.twelveMonthMoney && this.detail.currentCity.twelveMonthMoney != 0) {
        ...
        save() {
            ...
            insertCarSubscribe(this.detail).then(res => {
                if (res.code === 0) {
