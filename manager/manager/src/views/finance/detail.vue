<template>
    <div>
        <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item class="subscription-plan">{{ typeList[type] }}</el-breadcrumb-item>
        </el-breadcrumb>
        ...
        <div class="content" v-if="detail">
            ...
            <div class="container" v-if="type == 1 || type == 4 || type == 5">
                <div class="left">
                    <div class="item">
                        ...
                        <div class="item-content">￥{{ detail.costInfo.hasDepositFree === 2 ? 0 : `${ formatThousandNumber(detail.costInfo.deposit)}` }}   </div>
                    </div>
                    <div class="item" v-if="type == 1">
                        <div class="item-title">违章押金: </div>
                        <div class="item-content">￥{{ detail.costInfo.hasDepositFree === 2 ? `${ formatThousandNumber(violationDeposit)}` : 0 }}</div>
                    </div>
                    <div class="item" v-if="type == 4">
                        <div class="item-title">补缴押金: </div>
                        <div class="item-content">￥{{ formatThousandNumber(detail.costInfo.supplementaryDeposit || 0) }}</div>
                    </div>
                    <div class="item">
                        ...
                        <div class="item-content">
                            <div class="totalPay">
                                ￥<span class="price">
                                    {{ detail.costInfo.hasDepositFree === 2 ? formatThousandNumber(violationDeposit + (detail.costInfo.addedDeposit || 0)) : formatThousandNumber(detail.costInfo.deposit + (detail.costInfo.addedDeposit || 0) + (detail.costInfo.supplementaryDeposit || 0)) }}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="container" v-if="type == 2">
                <div class="left">
                    ...
                    <div class="item">
                        ...
                        <div class="item-content">￥{{ formatThousandNumber(detail.costInfo.premium)}}</div>
                    </div>
                    <div class="item">
                        <div class="item-title">共计费用: </div>
                        <div class="item-content">
                            <div class="totalPay">
                                ￥<span class="price">
                                    {{ formatThousandNumber(detail.costInfo.totalPayment + (detail.costInfo.totalPlateNumFee || 0) + (detail.costInfo.otherConfigPayment || 0) + (detail.costInfo.premium || 0)) }}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="right">
                    ...
                    <div class="item">
                        ...
                        <div class="item-content">{{ detail.costInfo.otherConfigPayment ? `￥${formatThousandNumber(detail.costInfo.otherConfigPayment)}` : '-' }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="container" v-if="type == 3">
                <div class="left">
                    <div class="item">
                        ...
                        <div class="item-content">
                            ￥{{ `${detail.costInfo.businessType === 1 ? formatThousandNumber(detail.costInfo.monthPayment) + '/月' : detail.costInfo.totalPayment}`}}
                        </div>
                    </div>
                    <div class="item">
                        <div class="item-title">共计费用: </div>
            ...
            <div class="title" v-if="detail.billList && detail.billList.length">账单已分{{detail.billList.length}}笔支付</div>
            <el-table 
                    ...
                    <template slot-scope="scope">
                        <div v-if="scope.row.payWay == 2">
                            <div class="operator" v-if="scope.row.confirmed == 0">
                                <div class="btn" @click="confirmSubscribeAccountDetailOfBank(scope.row, 1)">确认收款</div>
...
<script>
import { selectDepositDetail, selectSubscribeFeeDetail, selectAuctionDepositDetail, selectPageOfAccountDetail, confirmSubscribeAccountDetailOfBank, updateGuarantorInfo, updateDepositFreeApply } from '@/api/finance/finance'
...
export default {
  ...
  methods: {
    ...
    confirmSubscribeAccountDetailOfBank(row, confirmed) {
        this.$confirm(`确定要${confirmed == 1 ? '同意' : '驳回'}这笔银行转账吗？`, '提示', {
